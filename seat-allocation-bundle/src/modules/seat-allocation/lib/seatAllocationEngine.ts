import type { AllocationMode, Lab, ParsedRow } from '../types/seat';

export const rollKey = (value: string) => value.trim().toLowerCase();
...
export const compareSeatNumbers = (left: string, right: string): number => {
  ...
  const aHasStructured = Number.isFinite(a.number);
  const bHasStructured = Number.isFinite(b.number);
...
export const sortStudents = (students: ParsedRow[], mode: AllocationMode, seed: number): ParsedRow[] => {
  const list = [...students];

  if (mode === 'random') {
    // Deterministic PRNG for reproducible random mode runs in tests and sessions.
...
export const allocateSequentially = (params: {
  ...
  for (const lab of params.labs) {
    ...
    for (const seatNo of seatNumbers) {
      ...
      const summaryRow = seatSummary.find((item) => item.lab_id === lab.id);
      if (summaryRow) {
