...
import type { InvalidParsedRow, ParsedRow } from '../types/seat';
...
const normalizeCell = (value: unknown) => (typeof value === 'string' ? value.trim() : String(value ?? '').trim());
...
export const normalizeParsedRowsWithMapping = (
  ...
  const rollKey = mapping.rollKey;
  const departmentKey = mapping.departmentKey;
...
export const parseCsvTextRows = (text: string): Record<string, unknown>[] => {
  ...
  const tables = parseWorksheetTables(workbook.Sheets[sheetName], sheetName);
...
export const parseSpreadsheetFile = async (file: File): Promise<Record<string, unknown>[]> => {
  const tables = await parseSpreadsheetTables(file);
  return tables[0]?.rows ?? [];
};
...
export const parsePdfRows = async (file: File): Promise<Record<string, unknown>[]> => {
  ...
  for (const line of lines) {
    const tokens = line.split(/\s{2,}|\t|,\s*/).map((value) => value.trim()).filter(Boolean);
    if (tokens.length < 2) {
      continue;
    }
...
const parseWorksheetTables = (sheet: XLSX.WorkSheet, sheetName: string): SpreadsheetTableCandidate[] => {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    const hasValue = rowHasValue(matrix[i] ?? []);
    if (hasValue && start < 0) {
      start = i;
    }
    if (!hasValue && start >= 0) {
